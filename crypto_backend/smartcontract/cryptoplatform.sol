...
contract CryptoPlatform {
    ...
    // User-initiated buy: user pays gas via their wallet
    function buyCoin(string memory symbol, uint256 price, uint256 quantity) external onlyRegistered {
        ...
        emit CoinBought(msg.sender, symbol, quantity, totalCost);
    }
    ...
    // User-initiated sell: user pays gas via their wallet
    function sellCoin(string memory symbol, uint256 price, uint256 quantity) external onlyRegistered {
        User storage user = users[msg.sender];
        require(user.holdings[symbol] >= quantity, "Not enough coin to sell");
        user.holdings[symbol] -= quantity;
        user.usdBalance += price * quantity;
        ...
        emit CoinSold(msg.sender, symbol, quantity);
    }
    // ======= View =======
